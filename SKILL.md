...
→ **本步产出**（报告 §2）：访例记录**表格**，每行「对标 ｜ 来源 ｜ URL/路径 ｜ 凭什么被安装 ｜ 对我们的启示」（格式见 full-report.md §2）。

### 第三步 定式 · 生态定位
纵向（从什么痛点长出来、下一版往哪走）× 横向（同类凭什么立足：命名钩子 / 一句话定位 / 安装摩擦 / 首屏信任 / 可验证产物 / 安全边界 / 生态兼容 / 故事感）→ 交叉得出**一句话生态位**，不是泛泛总结。
→ **本步产出**（报告 §3）：一句话生态位 + 纵横交叉分析（full-report.md §3）。这句直接用作落成匾的「定式」栏与 README 定位句。
...
→ **本步产出**（报告 §5-6）：分级差距清单（P0/P1/P2，每条带预期提分）+ 三方向推荐及理由（full-report.md §5-6）。P0 清单即下一步细作的循环驱动表。
