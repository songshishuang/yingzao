...
- **2026-06-13 · v1.5.2** — **dogfood 反向修复两个 P0**（Y-011 · 用查勘档体检 20 个外部产品/运营 skill 反照自身）：① **死链检测 monorepo 误报**——内链 `grep -oE` 会从 `../../tools/REGISTRY.md` 提取子串当 skill 内路径，致 monorepo 上级共享资源被误判死链（churn-prevention / analytics 两案 FAIL 全系假阳性）；加前导边界 `(^|[^A-Za-z0-9._/-])` 排除 `../ ./ x/` 跨/上级引用，补 `tests/test-linkcheck.sh` 防回归（RED→GREEN：上级豁免 + 真死链仍检出 + 既有 bad-skill-demo 不破坏）；② **硬规则 #2 措辞歧义**——「无 A / 无 B / 无 C → ≤70」斜杠并列被误读为「满足其一即豁免」，致两评委对同一 skill 给分差 18，改写为「三者缺任一即触发上限」+ 补判例「有 evals.json 但无真实运行存档仍触发」。元收获：20 个外部 skill 全员 ≤70（无一带真实运行存档）实证「无实测进不了优秀区」核心主张成立。
- **2026-06-13 · v1.5.1** — **UTF-8 locale 健壮性修复**（darwin 独立实测发现，我单一 locale 终验漏掉）：① 12 处 shell `$VAR` 紧跟中文全角标点，在 UTF-8 + bash 3.2 下变量名被多字节字节污染 →`set -u` unbound 崩，全部加 `${}` 边界（self-integrity / check-release / gen-baseline / install）；② gen-baseline 的 `sort` 固定 `LC_ALL=C`——baseline.lock 行序须跨 locale 稳定，否则 `--check` 在异 locale 误判过期；③ CI 增 UTF-8 locale 实测步骤，堵掉「单一 locale 终验」盲区。教训：跨环境（locale）实测本是营造自家纪律，这次栽在没做、被一次外部 UTF-8 实测照出。
