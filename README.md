...
flowchart LR
    A["你的 skill"] --> B{"选档"}
    B -->|"查勘 · 10 分钟只读"| C["体检报告<br/>九维分 + P0/P1"]
    C -.->|"想认真修"| D
    B -->|"大修 · 全流程"| D["① 相地<br/>立项审查"]
    D --> E["② 访例<br/>同类调研"] --> F["③ 定式<br/>生态定位"] --> G["④ 勘验<br/>九维评分"] --> H["⑤ 画样<br/>差距规划"] --> I["⑥ 细作<br/>定向改写"] --> J["⑦ 落成<br/>验收交付"]
    J --> K(["岁修周期<br/>满 5 个自打磨"])
```
...
```mermaid
flowchart TD
    A["P0 差距清单"] --> B["改写一个变量<br/>（单候选 / 比样 3 候选 Pareto）"]
    B --> C["独立评委盲评<br/>（每轮换人防锚定）"]
    C --> D{"验证门：<br/>测试优于原版？"}
    D -->|"过门"| E["落盘 + 正反 patch 留底"]
    E --> F{"P0 清空 或<br/>单轮提升 <1 分？"}
    F -->|"否，继续"| B
    F -->|"是"| G(["收工 → 落成"])
    D -->|"不过门"| H["丢弃该轮（棘轮：分数只升不降）"]
    H --> I{"连续两轮不过？"}
    I -->|"否"| B
    I -->|"是"| J["落架 · 推倒重写<br/>（须你授权 · 同台对比<br/>赢了才换，输了恢复）"]
    J --> C
```
...
flowchart LR
    A["每次大修收尾"] --> B["30 秒复盘"]
    B --> C["新套路 / 新坑<br/>→ 台账 case-log<br/>（逐轮分数序列）"]
    B --> D["行为规则变更<br/>→ 候选 diff 等你确认"]
    C --> E{"台账满 5 个？"}
    E -->|"是"| F["岁修：用自己大修自己<br/>开源标准 · 陌生用户视角"]
    F --> G["实测数据校准九维权重"]
    G -.->|"尺子越用越准"| A
```
