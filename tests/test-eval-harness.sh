...
check det-pass            0 "det 确定性验证器：det_lift≥1 零回归 → 门①通过（D7·纯 det 不崩盲评聚合）"
check det-regress         1 "det：某题 pass_cand=0 而 pass_orig=1（回归）→ det 门不过（D7·棘轮）"
check det-mixed           0 "盲评+det 混合：两者都过才通过（D7·合并裁决·det 不污染盲评除零）"
